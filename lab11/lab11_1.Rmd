...
homerange %>% 
  ggplot(aes(x = log10.hra)) +
  geom_density(fill="honeydew2", alpha  =0.9, color = "salmon")+
  labs(title = "Distribution of Homerange")
```
...
homerange %>% 
  ggplot(aes(x=log10.mass)) +
  geom_histogram(aes(y = after_stat(density)), fill = "plum", alpha = 0.4, color = "cornsilk1")+
...
homerange %>%
           ...
           geom_bar(position="dodge", alpha=.7, color="black") +
